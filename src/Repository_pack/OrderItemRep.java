...
public class OrderItemRep implements Repository<OrderItem> {
    ...
    public void addItem(int orderId, int menuItemId, int quantity){
        try(Connection con = DatabaseConnection.getConnection()){
            ...
            ps.setInt(1, orderId); ps.setInt(2, menuItemId); ps.setInt(3, quantity);
            ps.executeUpdate();
    ...
    public List<OrderItem> findByOrderId(int orderId){
        ...
        try(Connection con = DatabaseConnection.getConnection()){
            ...
            while(rs.next()) list.add(new OrderItem(rs.getInt("order_id"), rs.getInt("menu_item_id"), rs.getInt("quantity")));
        } catch (Exception e){ throw new RuntimeException(e);}
        return list;
    }

    @Override public OrderItem findById(int id){
        return null;
    }
    @Override public List<OrderItem> findAll(){
        return List.of();
    }
    @Override public void save(OrderItem entity) {

    }
    @Override public void update(OrderItem entity) {
